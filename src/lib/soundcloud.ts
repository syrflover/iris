...
export const streamHeaderURLSC = curry(async (sc: SC, scURL: string) => {
    const track = ((await sc.tracks.getURL(scURL)) as unknown) as SCTrack;

    const { url: streamURL } = (await track.media.transcodings
        .iter()
